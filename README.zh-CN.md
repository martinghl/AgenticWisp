...
> **平台：** Linux 和 macOS 直接支持。Windows 需要走 **WSL** —— 中枢的生命周期命令(`up` / `down` / `status`)依赖 POSIX 进程语义(`os.kill`、`SIGTERM`、`start_new_session`),所以**原生 Windows（cmd / PowerShell）暂不支持**——请在 WSL 里跑。

```bash
# 安装(二选一)——[tui] 这个 extra 会拉入 textual 供花面板用:
