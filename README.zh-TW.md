...
> **平台：** Linux、macOS，或 Windows 透過 **WSL**。啟動腳本是 bash、預設按 Unix shell 來，所以**原生 Windows（cmd / PowerShell）跑不起來**——請在 WSL 裡跑。

```bash
git clone https://github.com/martinghl/AgenticWisp.git
cd AgenticWisp

# 選用：建個虛擬環境，啟動腳本會自動認出來（只有花俏的面板才需要 textual）
python3 -m venv .venv && .venv/bin/pip install textual

bin/wisp up          # 起中樞（自己轉背景；只綁 127.0.0.1）
bin/wisp watch       # 打開面板   （加 --simple 用純標準函式庫的簡版）
bin/wisp status      # 現在的彙總狀態
bin/wisp down        # 停中樞
```
