...
class UsageHUD(Static):
    """赛博朋克 Usage HUD;set_data(agg) 后渲染。黑底霓虹:亮字、暗数据雨底、扫描线、每模型能量条。"""
    ...
    def on_mount(self):
        self.set_interval(1 / 6, self.refresh)     # 6fps 动画(流光/扫描线/glitch)

    def set_data(self, agg):
        self._agg = agg

    def render(self):
        # 注意:render() 与 Widget._render()(textual 内部私有方法,绘制管线用它取 Visual)不同名但
        # 承担同样的职责 —— 必须返回 Visual(Content),若直接返回裸 rich.Text 会在挂载后的
        # Visual.to_strips 处崩溃(无 render_strips)。此处沿用 Task-5 的
        # Content.from_rich_text 方案兼容 textual 8.2.8。
    ...
    def _render_text(self):
        ...
        out.append(f"   IN {render.fmt_tokens(a.get('in', 0))} ▸ OUT {render.fmt_tokens(a.get('out', 0))}"
                   ...
                   f" ▸ {a.get('sessions', 0)} LIVE\n", style=palette.CYAN)
        # 每模型霓虹能量条(长度∝成本,流光)
        total = a.get("cost_usd", 0) or 1.0
        for r in a.get("by_model", [])[:3]:
            frac = ((r.get("cost_usd") or 0) / total) if total else 0.0
            bar = effects.energybar(frac, 22, t, color=palette.MAGENTA)
            out.append(f" [{render.short_model(r.get('model')):9s}] ", style=palette.CYAN)
            for ch, hexc in bar:
                out.append(ch, style=hexc)
            out.append(f"  {render.fmt_cost(r.get('cost_usd', 0))}\n", style=palette.AMBER)
        # 扫描线底纹一行
        out.append("═" * 40, style=palette.PURPLE)
        return out
